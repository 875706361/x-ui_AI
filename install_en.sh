...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/875706361/x-ui_AI/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}-english.tar.gz https://github.com/875706361/x-ui_AI/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/875706361/x-ui_AI/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz"
    ...
    cd x-ui
    chmod +x x-ui optimize.sh bin/xray-linux-${arch}
    cp -f x-ui.service /etc/systemd/system/
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/875706361/x-ui_AI/main/x-ui_en.sh
